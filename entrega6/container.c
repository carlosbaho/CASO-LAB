...
int func(void * a)
{
   ...
   pid = fork();
   if (pid == 0) execvp("cd", args->argument1);
...
int main(int argc, char * argv [])
{
   ...
   if (pid < 0) perror ("clone");
      ...
      char * cadena1 = "0 0 4294967295\n";
      char * cadena2 = "0 0 65535\n";
      char c [32];
      char d [32];
      sprintf(c, "/proc/%d/uid_map", pid);
      sprintf(d, "/proc/%d/gid_map", pid);
      FILE *fp = fopen(c, "w+");
      if (fp != NULL)
      {
         fputs(cadena1, fp);
